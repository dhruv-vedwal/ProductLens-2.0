...
def _trace() -> DemoTrace:
    ...
    trace.events.append(InteractionEvent(
        operation_id="submit", kind=OperationKind.SUBMIT, intent="Create record",
        occurred_at=datetime.now(UTC), action_at=datetime.now(UTC), success=False,
        after={"target_available": False}, duration_ms=100,
    ))
...
def test_read_only_evidence_repairs_only_the_dispatched_terminal_submit():
    repaired = repair_dispatched_terminal_submit(_trace(), {
