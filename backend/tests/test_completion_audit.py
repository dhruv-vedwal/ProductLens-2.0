...
def test_creation_audit_requires_matching_production_outcome_witness(tmp_path):
    ...
    (tmp_path / "objective.json").write_text(json.dumps({"permitted_mutations": ["create_isolated_record"]}))
    (tmp_path / "discovery" / "rehearsal-report.json").write_text(json.dumps({"outcome_target": {"name": "Appointment created"}}))
    (tmp_path / "execution" / "trace.json").write_text(json.dumps({"events": [{"kind": "Submit", "success": True, "after": {"text": "Save button"}}]}))
...
def test_audit_rejects_manifest_that_omits_required_evidence(tmp_path):
    ...
    (tmp_path / "artifact-manifest.json").write_text(json.dumps({
        "artifacts": [{
            "path": "final/demo.mp4", "bytes": video.stat().st_size,
