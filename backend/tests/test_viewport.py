...
def test_dense_application_uses_wider_capture_without_css_zoom():
    decision = choose_viewport(
        [_element(f"Action {number}") for number in range(45)], "Review actions"
    )
...
def test_full_walkthrough_does_not_assume_the_widest_native_capture_frame():
    ...
    assert (decision.viewport.width, decision.viewport.height, decision.browser_zoom_percent) == (
        1600,
        1000,
        100,
    )
