...
def test_cloud_source_timing_maps_trace_to_native_recording_evidence(monkeypatch, tmp_path: Path):
    ...
    frames.mkdir()
    (frames / "timing.json").write_text(json.dumps({"frames": [
        {"index": index, "received_at": (started + timedelta(seconds=second)).isoformat()}
    ...
    events = [
        InteractionEvent(
            operation_id=f"step-{index}", kind=OperationKind.CLICK, intent="Open section",
    ...
    trace = DemoTrace(run_id="timed", objective="Demo", started_at=started, recording_started_at=started, events=events)
    monkeypatch.setattr(
...
def test_cloud_source_timing_refuses_missing_clock_evidence(tmp_path: Path):
    ...
    recording = tmp_path / "browser-recording.mp4"
    recording.write_bytes(b"recording")
    trace = DemoTrace(run_id="missing", objective="Demo", started_at=started, recording_started_at=started)

    aligned, report = align_trace_to_recording(trace, recording=recording, screencast_frames=tmp_path / "none")
...
def test_editorial_cut_windows_expand_native_context_to_story_duration_floor():
    started = datetime.now(UTC)
    trace = DemoTrace(
        run_id="floor", objective="full walkthrough", started_at=started,
        recording_started_at=started,
        events=[
            InteractionEvent(
                operation_id="one", kind=OperationKind.CLICK, intent="Open detail",
                action_at=started + timedelta(seconds=20),
                occurred_at=started + timedelta(seconds=24), success=True, duration_ms=4_000,
            ),
            InteractionEvent(
                operation_id="two", kind=OperationKind.CLICK, intent="Show outcome",
                action_at=started + timedelta(seconds=80),
                occurred_at=started + timedelta(seconds=84), success=True, duration_ms=4_000,
            ),
        ],
    )
