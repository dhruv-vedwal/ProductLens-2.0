...
def _capability() -> ActionCapability:
    return ActionCapability(
        kind="form", purpose="appointment", source_url="https://example.test/appointments",
        entry_target=Target(name="New appointment", selector="#new"),
        form_schema=FormSchema(source_url="https://example.test/appointments", fields=[
            FormField(name="Customer", selector="#customer", control_type="text", required=True),
        ]),
...
def test_rehearsal_promotes_only_a_new_visible_success_witness():
    ...
    witnessed = derive_outcome_witness(
        ...
        observed=[ObservedElement(
            tag="div", role="status", name="Appointment created successfully",
            selector='[role="status"]', text="Appointment created successfully", source_url=capability.source_url,
        )],
    )
...
def test_rehearsal_does_not_treat_the_open_form_as_a_created_record():
    ...
    witnessed = derive_outcome_witness(
        ...
        observed=[ObservedElement(
            tag="div", role="dialog", name="Create appointment", text=opened_form,
...
def test_rehearsal_rejects_a_dialog_even_when_new_validation_copy_appears_after_submit():
    ...
    witnessed = derive_outcome_witness(
        ...
        observed=[ObservedElement(
            tag="div", role="dialog", name="Create appointment",
            text="Create appointment Customer Required field Save appointment",
            selector='[role="dialog"]', source_url=capability.source_url,
        )],
    )
    assert witnessed is None
...
def test_rehearsal_accepts_a_new_structural_row_with_the_generated_record_value():
    ...
    witnessed = derive_outcome_witness(
        ...
        observed=[ObservedElement(
            tag="tr", role="row", name="Maya Shah 985 223 9496 Pending",
            text="Maya Shah 985 223 9496 Pending", selector="tr",
            source_url=capability.source_url,
        )],
    )
...
def test_rehearsal_does_not_treat_a_non_result_container_with_a_generated_value_as_proof():
    ...
    witnessed = derive_outcome_witness(
        ...
        observed=[ObservedElement(
            tag="div", name="Maya Shah", text="Maya Shah", selector="div",
            source_url=capability.source_url,
        )],
    )
...
def test_rehearsal_accepts_a_stable_card_containing_the_unique_generated_value():
    ...
    witnessed = derive_outcome_witness(
        ...
        submitted_values=["Maya Shah"],
        observed=[ObservedElement(
            tag="div", name="Maya Shah Pending", text="Maya Shah Pending",
            selector='[data-testid="appointment-card"]', source_url=capability.source_url,
        )],
    )
...
def test_rehearsal_accepts_same_origin_post_submit_detail_navigation():
    ...
    assert witnessed.outcome_evidence[-1] == "rehearsal-visible-outcome:post-submit-detail-navigation"


def test_rehearsal_rejects_cross_origin_post_submit_navigation_as_a_creation_witness():
    assert _rehearsal_detail_navigation_witness(
        _capability(),
...
def test_verified_detail_navigation_becomes_a_url_postcondition_for_production():
    capability = _capability().model_copy(update={
        "verified": True,
...
def test_rehearsal_selects_the_feature_page_not_an_unrelated_form_by_probe_order():
    ...
    lead = _capability().model_copy(update={
        "purpose": "New lead", "source_url": lead_url,
    ...
    template = _capability().model_copy(update={
        "purpose": "New template", "source_url": template_url,
    ...
    context = ProductContext(
        ...
        objective=ObjectiveSpec(raw="Create an isolated lead-management demo record", primary_entity="lead management"),
        page_knowledge=[
            PageKnowledge(url=lead_url, title="Leads", purpose="Lead management", fingerprint="leads"),
            PageKnowledge(url=template_url, title="Lead templates", purpose="Reusable lead templates", fingerprint="templates"),
        ],
    )
...
def test_rehearsal_rejects_ambiguous_feature_forms_instead_of_guessing():
    ...
    context = ProductContext(
        url="https://example.test", title="Example", application_type="web_application",
        objective=ObjectiveSpec(raw="Create an isolated appointment demo record", primary_entity="appointment"),
        page_knowledge=[
            PageKnowledge(url="https://example.test/a", title="Appointment", purpose="Appointment", fingerprint="a"),
            PageKnowledge(url="https://example.test/b", title="Appointment", purpose="Appointment", fingerprint="b"),
        ],
    )
...
def test_rehearsal_omits_optional_selectors_without_observed_choices():
    capability = _capability().model_copy(update={
        "form_schema": FormSchema(
            ...
            fields=[
                FormField(name="Branch (Optional)", selector="#branch", control_type="combobox"),
                FormField(name="Customer", selector="#customer", control_type="text"),
            ],
        ),
    })
    ...
    assert [operation.target.name for operation in operations] == [
        "New appointment", "Customer", "Save appointment",
    ]
    ...
    capability = _capability().model_copy(update={
        "form_schema": FormSchema(
            ...
            fields=[
                FormField(name="Customer", selector="#customer", control_type="text"),
                FormField(
                    name="Branch (Optional)", selector="#branch", control_type="combobox", options=["North"],
                ),
            ],
        ),
    })
...
def test_rehearsal_still_rejects_required_selector_without_observed_choices():
    capability = _capability().model_copy(update={
        "form_schema": FormSchema(
            source_url="https://example.test/appointments",
            fields=[FormField(name="Branch", selector="#branch", control_type="combobox", required=True)],
        ),
    })
...
def test_rehearsal_rejects_a_schema_with_visible_but_unresolved_required_controls():
    capability = _capability().model_copy(update={
        "form_schema": FormSchema(
            source_url="https://example.test/appointments",
            fields=[FormField(name="Owner (Optional)", selector="#owner", control_type="combobox")],
