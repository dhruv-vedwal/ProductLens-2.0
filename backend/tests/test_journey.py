...
def test_journey_requires_context_and_exploration_after_navigation():
    ...
    scenes = [
        {"event_id": event.id, "show_cursor": True, "caption_safe_zone": "bottom"}
...
def test_journey_rejects_route_sweep_without_page_exploration():
    ...
    assert (
        "JOURNEY_HAS_NAVIGATION_WITHOUT_EXPLORATION"
        in inspect_journey(build_journey(trace, scenes))["hard_failures"]
    )
...
def test_full_walkthrough_rejects_an_opening_page_that_is_only_glimpsed_before_navigation():
    trace = DemoTrace(
        run_id="opening-gap",
        objective="Complete walkthrough of the product",
        started_at=datetime.now(UTC),
        events=[
            _event("opening", OperationKind.VERIFY_STATE, "https://example.test/"),
...
def test_journey_rejects_a_return_to_an_already_completed_page():
    trace = DemoTrace(
        ...
        events=[
            ...
            _event(
                "second-page", OperationKind.OPEN_NAVIGATION_ITEM, "https://example.test/second"
            ),
    ...
    assert (
        "JOURNEY_REVISITS_COMPLETED_PAGE"
        in inspect_journey(build_journey(trace, scenes))["hard_failures"]
    )
...
def test_journey_allows_reentry_when_it_proves_a_meaningful_validated_interaction():
    ...
    report = inspect_journey(
        build_journey(trace, [{"event_id": event.id} for event in trace.events])
    )
    ...
    scenes = [
        {
            "event_id": event.id,
            "dwell_seconds": 2,
            "required_content_groups": ["Current role", "Internship"],
            "covered_content_groups": (
                ["Current role"] if event.operation_id == "current-role" else []
            ),
        }
        for event in trace.events
    ]
...
def test_journey_does_not_invent_scroll_motion_for_an_already_visible_landmark():
    ...
    trace = DemoTrace(
        run_id="visible", objective="portfolio", started_at=datetime.now(UTC), events=[event]
    )
...
def test_journey_requires_continuity_for_a_real_browser_scroll():
    ...
    trace = DemoTrace(
        run_id="moved", objective="portfolio", started_at=datetime.now(UTC), events=[event]
    )
