...
def test_sample_benchmark_derives_measured_and_editorial_envelopes(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        ...
        lambda path: {
            "file": path.name,
            "codec": "h264",
            "width": 1920 if path.stem == "one" else 1280,
            "height": 1080 if path.stem == "one" else 720,
            "duration_seconds": 90 if path.stem == "one" else 120,
            "frame_rate": 30.0,
...
def test_benchmark_comparison_does_not_claim_editorial_parity_from_media_probe(
    ...
    benchmark = {
        ...
        "measured_envelope": {
            "minimum_width": 1280,
            "minimum_height": 720,
            "minimum_frame_rate": 30.0,
            "shortest_duration_seconds": 75.0,
        },
        "quality_target": {
            "width": 1920,
            "height": 1080,
...
def test_benchmark_comparison_rejects_output_below_measured_sample_envelope(
    monkeypatch, tmp_path: Path
):
    output = tmp_path / "output.mp4"
    output.write_bytes(b"output")
    ...
    report = compare_to_sample_benchmark(
        output,
        {
            "measured_envelope": {
                "minimum_width": 1280,
    ...
    assert set(report["hard_failures"]) == {
        "BELOW_SAMPLE_RESOLUTION_ENVELOPE",
        "BELOW_SAMPLE_FRAME_RATE_ENVELOPE",
        "BELOW_SAMPLE_DURATION_ENVELOPE",
    }
