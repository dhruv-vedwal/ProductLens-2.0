...
def test_page_compiler_does_not_turn_table_chrome_into_fake_scroll_scenes():
    ...
    context = ProductContext(
        url=root, title="Example", application_type="dashboard", confidence=1,
        page_knowledge=[PageKnowledge(
            url=root, title="Records", purpose="Records", fingerprint="records",
        ...
        elements=[
            ...
            ObservedElement(tag="input", name="Search records", selector="input", source_url=root, actionable=True),
            ObservedElement(tag="button", name="Reminders", selector="#reminders", source_url=root, actionable=True),
            ObservedElement(tag="th", name="CREATION DATE", selector="th:nth-child(1)", source_url=root),
            ObservedElement(tag="th", name="RECORD NAME", selector="th:nth-child(2)", source_url=root),
        ],
    )
    proposal = build_page_complete_proposal(
        context, CandidateDemoFlow(name="records", page_urls=[root], score=1),
    )
    ...
    scoped = context().model_copy(update={
        "objective": ObjectiveSpec(
            ...
            supporting_relationships=[{
                "source": "Appointment Configuration", "target": "Appointment workflow",
            }],
        ),
        "page_knowledge": [
            PageKnowledge(
                url="https://example.test/appointments", title="Appointment workflow",
                purpose="Appointment workflow", visible_sections=["Schedule appointments"],
                fingerprint="appointments",
            )
        ],
    })
    candidate = CandidateDemoFlow(
        name="appointments", page_urls=["https://example.test/appointments"],
        rationale=["feature"], expected_outcomes=["appointment"], evidence_coverage=["page"], score=0.8,
    )
...
def test_authenticated_context_satisfies_login_must_show_requirement():
    ...
    scoped = context().model_copy(update={
        ...
        "page_knowledge": [PageKnowledge(
            url=root, title="Records", purpose="Records workspace",
            visible_sections=["Records"],
    ...
    candidate = CandidateDemoFlow(
        name="records", page_urls=[root], rationale=["workspace"],
        expected_outcomes=["records"], evidence_coverage=["page"], score=0.8,
    )
    ProductionPlanningService._validate_objective_grounding(scoped, candidate)
...
def test_planner_rejects_settings_shell_as_relationship_detail_evidence():
    scoped = context().model_copy(update={
        ...
        "page_knowledge": [
            PageKnowledge(
                url="https://example.test/leads", title="Lead Management", purpose="Lead workspace",
                visible_sections=["Lead list"], fingerprint="leads",
            ),
            PageKnowledge(
                url="https://example.test/settings", title="Settings", purpose="Settings dashboard",
                # A global shell may expose this label, but it is not the
                # configuration page itself.
                visible_sections=["Lead Configuration"], fingerprint="settings",
            ),
        ],
    })
    candidate = CandidateDemoFlow(
        name="lead context", page_urls=["https://example.test/leads", "https://example.test/settings"],
        rationale=["context"], expected_outcomes=["lead"], evidence_coverage=["page"], score=.8,
    )
...
def test_planner_accepts_a_semantic_relationship_when_workspace_uses_a_product_label():
    ...
    scoped = context().model_copy(update={
        ...
        "page_knowledge": [
            PageKnowledge(
                url="https://example.test/customers-v3", title="Customers V3", purpose="Customers",
                visible_sections=["Customer list"], fingerprint="customers",
            ),
            PageKnowledge(
                url="https://example.test/settings/customer/configuration", title="Settings",
                purpose="Configuration", visible_sections=["Customer Configuration"],
    ...
    candidate = CandidateDemoFlow(
        ...
        page_urls=["https://example.test/customers-v3", "https://example.test/settings/customer/configuration"],
        rationale=["context"], expected_outcomes=["customer"], evidence_coverage=["page"], score=.8,
    )
...
def test_planner_accepts_editorial_must_show_phrases_when_page_evidence_is_terse():
    ...
    scoped = context().model_copy(update={
        "objective": ObjectiveSpec(
            ...
            must_show=["home identity and capabilities", "every meaningful featured project on Home"],
            supporting_relationships=[{"source": "Home", "target": "Timeline"}],
        ),
        "page_knowledge": [
            PageKnowledge(
                url=root, title="Jordan Lee | Product Engineer", purpose="Portfolio overview",
                visible_sections=["Core Capabilities", "Featured Projects"],
                visible_facts=["Atlas is a scheduling workspace for distributed teams."], fingerprint="home",
            ),
            PageKnowledge(
                url=timeline, title="Professional Timeline", purpose="Career history",
                visible_sections=["Experience"], visible_facts=["Career progression and contributions."],
                fingerprint="timeline",
            ),
        ],
    })
    candidate = CandidateDemoFlow(
        name="portfolio walkthrough", page_urls=[root, timeline], score=.9,
    )
    ProductionPlanningService._validate_objective_grounding(scoped, candidate)
...
def test_planner_still_rejects_an_entirely_unsupported_must_show_phrase():
    scoped = context().model_copy(update={
        "objective": ObjectiveSpec(raw="Show the product", must_show=["billing reconciliation"]),
        "page_knowledge": [PageKnowledge(
            url="https://example.test/", title="Product", purpose="Overview",
            visible_sections=["Dashboard"], fingerprint="home",
        )],
    })
    candidate = CandidateDemoFlow(name="overview", page_urls=["https://example.test/"], score=.9)
...
def test_full_walkthrough_treats_explanatory_product_purpose_as_story_intent():
    ...
    scoped = context().model_copy(update={
        ...
        "page_knowledge": [PageKnowledge(
            url=root, title="Study Planner", purpose="Plan study work and track progress",
            visible_sections=["Today", "Progress"], fingerprint="study-planner",
        ), PageKnowledge(
            url="https://example.test/progress", title="Progress", purpose="Track progress",
            visible_sections=["Progress"], fingerprint="progress",
        )],
    })
    candidate = CandidateDemoFlow(name="complete walkthrough", page_urls=[root, "https://example.test/progress"], score=.9)
    ProductionPlanningService._validate_objective_grounding(scoped, candidate)
...
def test_planner_rejects_one_configuration_page_as_proof_of_both_relationship_sides():
    scoped = context().model_copy(update={
        ...
        "page_knowledge": [
            PageKnowledge(
                url="https://example.test/settings/customer/configuration", title="Customer Config",
                purpose="Customer configuration", fingerprint="customer-config",
            ),
        ],
    })
    candidate = CandidateDemoFlow(
        name="configuration only", page_urls=["https://example.test/settings/customer/configuration"],
        rationale=["context"], expected_outcomes=["customer"], evidence_coverage=["page"], score=.8,
    )
...
def test_planner_grounds_context_from_exploration_without_turning_it_into_a_video_page():
    ...
    scoped = context().model_copy(update={
        ...
        "page_knowledge": [
            PageKnowledge(url=workspace, title="Customers", purpose="Customer workspace", fingerprint="customers"),
            PageKnowledge(url=configuration, title="Configuration", purpose="Customer configuration", fingerprint="configuration"),
        ],
    })
    candidate = CandidateDemoFlow(
        name="customer walkthrough", page_urls=[workspace], supporting_page_urls=[configuration],
        rationale=["context"], expected_outcomes=["customer"], evidence_coverage=["page"], score=.8,
    )
...
def test_verified_form_transition_returns_to_the_capability_page_before_opening_it():
    ...
    context_with_navigation = ProductContext(
        url=leads, title="Example", application_type="crm",
        navigation=[ObservedElement(
            tag="a", name="Leads", selector="#leads", href="/leads",
            source_url=configuration, actionable=True, navigation_scope="primary",
        )],
    )
    proposal = WorkflowProposal(
        narrative_goal="context first", selected_workflow="lead context", expected_outcomes=["configuration"],
        steps=[SemanticOperation(
            kind=OperationKind.NAVIGATE, intent="Open configuration", value=configuration,
            postconditions=[Postcondition(kind="url", expected=configuration)], page_url=configuration,
        )],
    )
...
async def test_authorized_generic_record_creation_requires_and_compiles_verified_capability():
    entry = Target(name="Create record", selector="#create-record", source_url="https://example.test/app")
    capability = ActionCapability(
        kind="form", purpose="record", source_url="https://example.test/app",
        entry_target=entry,
        form_schema=FormSchema(source_url="https://example.test/app", fields=[
            FormField(name="Customer email", selector="#email", control_type="email", required=True),
        ]),
        submit_target=Target(name="Save record", selector="#save", source_url="https://example.test/app"),
        outcome_target=Target(name="Record created", selector="#created", source_url="https://example.test/app"),
        outcome_evidence=["visible confirmation"], verified=True,
    )
    proposal = WorkflowProposal(
        narrative_goal="Show the record workflow", selected_workflow="record",
        steps=[SemanticOperation(
            kind=OperationKind.CLICK, intent="Establish record workspace", target=entry,
            postconditions=[Postcondition(kind="visible", expected=True, target=entry)],
        )], expected_outcomes=["record workspace"],
    )
    scoped = context().model_copy(update={
        "elements": [ObservedElement(tag="button", name="Create record", selector="#create-record", source_url="https://example.test/app")],
        "objective": ObjectiveSpec(raw="Create an isolated demo record", permitted_mutations=["create_isolated_record"]),
        "capabilities": [capability.model_dump(mode="json")],
    })
    plan = await ProductionPlanningService(StubPlanner(proposal)).plan(
        objective="Create an isolated demo record", context=scoped, allow_external_side_effects=True,
    )
...
async def test_authorized_creation_rejects_a_form_without_independent_outcome_proof():
    proposal = WorkflowProposal(
        narrative_goal="Show records", selected_workflow="record",
        steps=[SemanticOperation(kind=OperationKind.CLICK, intent="Open records", target=Target(name="Create record", selector="#create-record"), postconditions=[Postcondition(kind="visible", expected=True, target=Target(name="Create record", selector="#create-record"))])],
        expected_outcomes=["records"],
    )
    scoped = context().model_copy(update={
        "elements": [ObservedElement(tag="button", name="Create record", selector="#create-record")],
        "objective": ObjectiveSpec(raw="Create a demo record", permitted_mutations=["create_isolated_record"]),
        "capabilities": [ActionCapability(
            kind="form", purpose="record", source_url="https://example.test/app",
            entry_target=Target(name="Create record", selector="#create-record"),
            form_schema=FormSchema(source_url="https://example.test/app", fields=[FormField(name="Name", selector="#name", control_type="text")]),
            submit_target=Target(name="Save", selector="#save"), verified=True,
    ...
    with pytest.raises(PlanningValidationError, match="independently verified form capability"):
        await ProductionPlanningService(StubPlanner(proposal)).plan(
            objective="Create a demo record", context=scoped, allow_external_side_effects=True,
        )
...
async def test_authorized_creation_uses_the_selected_feature_capability_not_first_probe():
    ...
    lead = ActionCapability(
        kind="form", purpose="New lead", source_url=lead_url,
        entry_target=Target(name="New lead", selector="#new-lead", source_url=lead_url),
        form_schema=FormSchema(source_url=lead_url, fields=[FormField(name="Name", selector='[name="name"]', control_type="text", required=True)]),
        ...
        outcome_evidence=["visible lead confirmation"], verified=True,
    )
    template = ActionCapability(
        kind="form", purpose="New template", source_url=template_url,
        entry_target=Target(name="New template", selector="#new-template", source_url=template_url),
        form_schema=FormSchema(source_url=template_url, fields=[FormField(name="Template name", selector='[name="template"]', control_type="text", required=True)]),
        submit_target=Target(name="Save template", selector="#save-template", source_url=template_url),
        outcome_target=Target(name="Template created", selector="#template-created", source_url=template_url),
        outcome_evidence=["visible template confirmation"], verified=True,
    )
    proposal = WorkflowProposal(
        narrative_goal="Show lead management", selected_workflow="lead management",
        steps=[SemanticOperation(kind=OperationKind.CLICK, intent="Establish leads", target=lead.entry_target)],
        expected_outcomes=["lead workspace"],
    )
    scoped = ProductContext(
        url=lead_url, title="Example", application_type="web_application",
        objective=ObjectiveSpec(raw="Create an isolated lead management record", primary_entity="lead management", permitted_mutations=["create_isolated_record"]),
        elements=[ObservedElement(tag="button", name="New lead", selector="#new-lead", source_url=lead_url)],
        page_knowledge=[
            PageKnowledge(url=lead_url, title="Leads", purpose="Lead management", visible_sections=["Lead list and details"], fingerprint="leads"),
        ],
        candidate_demo_flows=[CandidateDemoFlow(
            name="lead management", page_urls=[lead_url], rationale=["requested feature"],
            expected_outcomes=["lead workflow"], evidence_coverage=["lead page"], score=0.95,
        )],
        capabilities=[template.model_dump(mode="json"), lead.model_dump(mode="json")],
    )
    plan = await ProductionPlanningService(StubPlanner(proposal)).plan(
        objective="Create an isolated lead management record", context=scoped, allow_external_side_effects=True,
    )
...
async def test_planner_rejects_a_sparse_evidence_candidate_before_production_capture():
    ...
    scoped = ProductContext(
        url=records, title="Example", application_type="dashboard",
        objective=ObjectiveSpec(
            raw="Create a one minute walkthrough of records",
            primary_entity="records", minimum_duration_seconds=60,
            target_duration_seconds=90, maximum_duration_seconds=120,
        ),
        page_knowledge=[PageKnowledge(
        ...
        candidate_demo_flows=[CandidateDemoFlow(
            name="sparse records", page_urls=[records], expected_outcomes=["Records"],
            estimated_duration_seconds=20, evidence_coverage=["page:https://example.test/records"], score=0.9,
        )],
    )
    proposal = WorkflowProposal(
        narrative_goal="Show records", selected_workflow="records",
        steps=[SemanticOperation(kind=OperationKind.VERIFY_STATE, intent="Establish records", target=Target(name="Records"))],
        expected_outcomes=["Records"],
    )

    with pytest.raises(PlanningValidationError, match="lacks enough evidence-backed content"):
        await ProductionPlanningService(StubPlanner(proposal)).plan(
            objective=scoped.objective.raw, context=scoped,
            target_duration_seconds=90, minimum_duration_seconds=60, maximum_duration_seconds=120,
        )
...
async def test_persisted_candidate_summary_is_reconciled_to_compiled_operations():
    ...
    plan = await ProductionPlanningService(StubPlanner(proposal)).plan(
        objective="Show records", context=scoped, target_duration_seconds=90,
    )
    ...
    assert any(ref.startswith("page:https://example.test/records") for ref in selected["evidence_coverage"])
...
def test_complete_home_keeps_all_meaningful_project_and_outcome_landmarks():
    ...
    root = "https://example.test/"
    names = [
        "Identity", "Capabilities", "Realtime Chat", "PromptRouter", "CalmPrep",
        "CookPlan AI", "No-show reduction", "Dashboard speedup", "Revenue growth",
        "Processing throughput", "Architecture",
    ]
    scoped = context().model_copy(update={
        "url": root,
        "page_knowledge": [PageKnowledge(
            ...
            visible_facts=[f"{name} :: {name} is an observed product capability or outcome with useful context." for name in names],
...
def test_full_walkthrough_keeps_each_distinct_role_on_a_multi_role_page():
    ...
    scoped = context().model_copy(update={
        "url": root,
        "page_knowledge": [
            PageKnowledge(url=root, title="Home", purpose="Home", visible_sections=["Home"], fingerprint="home"),
            PageKnowledge(
                url=timeline, title="Timeline", purpose="Professional History",
                scroll_landmarks=["Professional History", "Current Role", "Previous Internship", "Education"],
                visible_sections=["Professional History", "Current Role", "Previous Internship", "Education"],
                visible_facts=[
        ...
        "elements": [
            ObservedElement(tag="a", name="Timeline", selector="#timeline", href="/timeline", source_url=root, actionable=True, navigation_scope="primary"),
            *[
                ObservedElement(tag="h2", name=name, selector=f"#{index}", source_url=timeline, text=name)
                for index, name in enumerate(["Professional History", "Current Role", "Previous Internship", "Education"])
            ],
        ],
    })
    candidate = CandidateDemoFlow(name="complete evidence walkthrough", page_urls=[root, timeline], score=1)
    proposal = build_page_complete_proposal(scoped, candidate)
