...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read-only visible synthetic-record presence check")
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            for field, value in dataset.items():
                ...
                present.append({"field": field, "visible_match": len(compact) >= 5 and compact in visible_compact})
            return {
                "run_id": args.run_id,
                "source_url": capability.source_url,
                "checked_fields": present,
                "any_visible_match": any(item["visible_match"] for item in present),
                "result": "visible_match" if any(item["visible_match"] for item in present) else "inconclusive",
            }
        finally:
